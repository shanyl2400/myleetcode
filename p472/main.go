...
func (root *treeNode) insert(word string) {
	node := root
	for _, ch := range word {
		ch -= 'a'
...
func (root *treeNode) dfs(word string) bool {
	if word == "" {
		return true
	}
	node := root
